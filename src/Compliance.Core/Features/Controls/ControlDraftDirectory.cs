...
static class ControlDraftDirectorySchema
{
...
sealed class FitzControlDraftDirectory(IKvClient client)
    : FitzKvProjectionStore(client, "kv://bdgrz/control-draft-directory/projection",
            "ControlDraftDirectory"),
      IControlDraftDirectoryReader, IControlDraftDirectoryProjection
{
    public ValueTask<ProjectionCheckpoint> LoadCheckpointAsync(Uuid tenantId,
        CancellationToken ct = default) =>
        base.LoadCheckpointAsync(new CheckpointIdentity("ControlDraftDirectory",
    ...
    public async ValueTask ApplyAsync(DomainEvent domainEvent, CancellationToken ct = default)
    {
        switch (domainEvent)
        {
            case ControlDraftCreated created:
                await ControlDraftDirectorySchema.Controls.InsertAsync(Transaction,
                    new ControlDraftView(created.TenantId, created.ProgramId, created.ControlId,
                        created.Identifier, 1, "draft", "unresolved", "unresolved",
                        created.Content, created.ActorMemberId, created.ActorDisplay,
                        created.ChangedAt), ct).ConfigureAwait(false);
                await ControlDraftDirectorySchema.Revisions.InsertAsync(Transaction,
            ...
            case ControlDraftRevised revised:
                var current = await ControlDraftDirectorySchema.Controls.GetAsync(Transaction,
                ...
                await ControlDraftDirectorySchema.Controls.ReplaceAsync(Transaction, current,
                    current with
                    {
        ...
        CancellationToken ct = default)
    {
        ...
        return await ControlDraftDirectorySchema.Controls.GetAsync(tx, controlId, ct)
            .ConfigureAwait(false);
    }
    ...
    {
        await using var tx = await BeginReadAsync(tenantId.ToString(), ct).ConfigureAwait(false);
        return await ControlDraftDirectorySchema.Controls.QueryAsync(tx,
            ControlDraftDirectorySchema.ByProgramIdentifier.Query()
    ...
    {
        await using var tx = await BeginReadAsync(tenantId.ToString(), ct).ConfigureAwait(false);
        return await ControlDraftDirectorySchema.Revisions.GetAsync(tx,
            ControlDraftDirectorySchema.RevisionKey(controlId, revision), ct)
            .ConfigureAwait(false);
    }
}
