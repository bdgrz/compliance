...
| --- | --- | --- |
| Operator-only tenant creation and no implicit operator membership | `RegisterTenantAuthorizerTests`, `RegisterTenantRequestScenarioTests`, `TenantInvitationE2ETests.FirstAdministratorMustVerifyAndAcceptBeforeTenantAccess`, `SplitHostTenantE2ETests.IndependentWorkerCompletesFirstAdministratorAndSlugFlow` | Production operator IDs must be explicitly configured. |
