...
| Automated evidence | The client test suite runs axe-core against rendered pages with the WCAG 2.2 AA rule tags, and runs the document-level rules (language, title, viewport) against the application shell. Zero violations is required, and it runs in CI through `npm run client:check`. jsdom has no layout, so color contrast and target size are excluded from the automated run and checked manually. Results axe reports as incomplete are not failures; the manual pass covers them. |
| Manual evidence | Each frontend child's PR links a record of every item in the manual checklist in [accessibility and browser support](../accessibility-and-browser-support.md#manual): keyboard operation, visible focus, zoom and reflow, contrast, target size, reduced motion, error identification and announcement, a per-route document title, and a screen-reader walkthrough with NVDA on Firefox or Chrome and VoiceOver on Safari. |
