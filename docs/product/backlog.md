...
Bundle dependent backend children into a PR when they form one reviewable capability and share contracts, domain records, or acceptance tests. List every covered child and its specific acceptance evidence in the PR. Run the full applicable gate once on the final head, then close only the children whose backend criteria passed. Do not split a capability into PRs for individual tests or layers. The first bundle covers the R1-15 remainder, EN-01, and R1-01; its tracking issues are #152, #157, and #158.
