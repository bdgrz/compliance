...
| Tenant invitations | Each owner sees only their own two invitation emails while traversing `limit=1` cursors. Foreign tenant paths fail; filtering the owned tenant with the other tenant's email returns an empty page. | Delivery and personal acceptance have separate lifecycle evidence; this scenario does not assert either. |
