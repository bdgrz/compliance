...
| Application import batch GET, `/rows`, `/preview` and matching MCP reads | `TenantReadLeakMatrixE2ETests.ShouldScopeImportCountsRowsAndPreviewGivenTwoTenants`: both modes; separate batch row/invalid/progress counters, row and preview content, HTTP row cursor and foreign-cursor rejection, cross-tenant IDs, outsider denial. | Add preview and MCP row cursor variants. No import list or separate count route exists. |
...
or artifact-content route. Import batch status carries the implemented
row/invalid/progress counts. Add any future surface to this matrix in the
