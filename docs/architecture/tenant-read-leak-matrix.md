...
| Program and client-service current, revision, list, history and setup-work reads | [PR #370](https://github.com/bdgrz/compliance/pull/370) is pending merge. Its `ProgramReadLeakMatrixE2ETests.ShouldScopeProgramAndServicePagesGivenTwoTenants` covers HTTP/MCP exact current and revision reads, six paged list/history/setup-work families, seeded content and versions, strict foreign-cursor rejection, foreign IDs and outsider denial in both modes. | Do not count this row as merged evidence until #370 lands. The fixture does not cover stopped-worker replay, Program stage transitions or future setup-work inputs beyond boundaries. These routes expose no separate count or search operation. |
...
list cursor and the remaining application contexts above. Program and
client-service reads remain pending #370 merge. Existing capability tests
often prove standalone cross-tenant denial or split-host recovery, but that
